...
ラスボスは時間経過ではなく**ミニボス5体を順に撃破後**に出現する（`CLEAR_TIME = 540` は敵の強さスケーリングが頭打ちになる秒数で、出現タイミングとは別）。`warp=` は敵を強くするだけでミニボス討伐は進めないため、ラスボス到達には `demo=1` である程度の時間プレイさせてミニボスを倒させる必要がある（`godmode=1` を付けると道中で死なずに到達しやすい）。
